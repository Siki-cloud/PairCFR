...
class SupConLossv2(nn.Module):
    ...
    def __init__(self, num_classes=2, lambda_=0.8, beta=1.,distance="cosine", device=torch.device("cuda"), temperature=0.3,
                 contrast_mode='all',except_1=True,
                 ):
        ...
        self.except_1 = except_1
        if except_1:  # default filter the neutral label
            print("v2supconloss except label = 1")
    ...
    def forward(self, output, labels, mask=None,no_grad=True):
        ...
        if (labels == 2).any().item() and self.except_1:
            ...
            neg_mask = neg_mask * mask_neutural 
            neg_mask[mask_neutural_index ,:]= 0.
        
        exp_contrast = torch.exp(anchor_dot_contrast)* neg_mask 
        log_prob = anchor_dot_contrast - torch.log(exp_contrast.sum(1, keepdim=True)+torch.exp(anchor_dot_contrast)+eps)
        ...
        # contrastive_loss = mean_log_prob_pos.sum()
